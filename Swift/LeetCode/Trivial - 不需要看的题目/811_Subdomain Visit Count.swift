...
class Num811 {
  // 思路很明白，实现细节题
  // 重点是掌握split, joined 之类的string，array操作
  func subdomainVisits(_ cpdomains: [String]) -> [String] {

    // for each visit, needs to update count for each domain
    // such as 9001 discuss.leetcode.com
    // ->
    // [discuss.leetcode.com: 9001]
    // [leetcode.com: 9001]
    // [com: 9001]

    // If there's another visit
    // first check the dictionary and update the count accordingly

    var record: [String: Int] = [:]

    for visit in cpdomains {
      let components = visit.split(separator: " ")
      let count = Int(components[0])!

      let address = String(components[1])
      // iterate the subdomains and update the record
