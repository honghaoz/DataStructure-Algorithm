...
class Num401 {
  ...
  func readBinaryWatch(_ num: Int) -> [String] {
    ...
    for i in 0...num { // i is the hour light on count
      let hourNumbers = generate(hours, i)
      let minuteNumbers = generate(minutes, num - i)
      for h in hourNumbers {
        guard 0 <= h, h <= 11 else {
          continue
        }
        for m in minuteNumbers {
          guard 0 <= m, m <= 59 else {
            continue
          }

          let time = "\(h):" + String(format: "%02d", m)
          result.append(time)
        }
      }
    }
  ...
  // For possible numbers and led light on count, find the all possible numbers
  private func generate(_ nums: [Int], _ ledCount: Int) -> [Int] {
    var result: [Int] = []
    backtrack(nums, 0, ledCount, 0, &result)
    return result
  }

  // nums to choose + start index -> get the nums to choose
  // count is the how many numbers to choose
  // sum is the current sum
  private func backtrack(_ nums: [Int], _ startIndex: Int, _ count: Int, _ sum: Int, _ result: inout [Int]) {
    if count == 0 {
      result.append(sum)
      return
    }
